 ...
 * for the multiplayer Corridor game.
 */
...
export class GameRoomManager {
     ...
     * 2. Initializes socket.data.currentRoom to track room membership
    ...
    private handleConnection(socket: Socket): void {
        ...
        // Set up event handlers using socket.data for room tracking
        socket.on(SocketEvents.JOIN_ROOM, async (
            roomName: string,
            callback: (response: JoinRoomResponse) => void
        ) => {
            socket.data.currentRoom = await this.handleJoinRoom(socket, roomName, socket.data.currentRoom, callback);
        });

        socket.on(SocketEvents.LEAVE_ROOM, (callback?: (response: LeaveRoomResponse) => void) => {
            socket.data.currentRoom = this.handleLeaveRoom(socket, socket.data.currentRoom, callback);
        });

        socket.on(SocketEvents.GAME_STATE, (state: unknown) => {
            this.handleGameState(socket, socket.data.currentRoom, state);
        });

        socket.on(SocketEvents.REQUEST_STATE, () => {
            this.handleRequestState(socket, socket.data.currentRoom);
        });

        socket.on(SocketEvents.SEND_STATE, (data: SendStatePayload) => {
            this.handleSendState(socket, socket.data.currentRoom, data);
        });

        socket.on('disconnect', () => {
            this.handleDisconnect(socket, socket.data.currentRoom);
        });
    }
