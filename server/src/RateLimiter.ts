 ...
 * @param key - The identifier to rate limit (socket ID or IP)
 * @returns true if allowed, false if rate limited
 */
async function checkLimit(limiter: RateLimiterMemory, key: string): Promise<boolean> {
    try {
...
export const checkJoinRoomLimit = (socketId: string) => checkLimit(joinRoomLimiter, socketId);

/** Checks if a GAME_STATE request is within rate limits. */
export const checkGameStateLimit = (socketId: string) => checkLimit(gameStateLimiter, socketId);

/** Checks if a SEND_STATE request is within rate limits. */
export const checkSendStateLimit = (socketId: string) => checkLimit(sendStateLimiter, socketId);

/** Checks if a REQUEST_STATE request is within rate limits. */
export const checkRequestStateLimit = (socketId: string) => checkLimit(requestStateLimiter, socketId);
